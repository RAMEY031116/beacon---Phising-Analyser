...
# Security-link wrapper support

Yeti can recognise supported rewritten security links before it scores the destination.

For **LinkScan** URLs, Yeti attempts to reveal the destination embedded in the rewritten link, then analyses that destination rather than treating `linkscan.io` as the final website. This is useful when an email security service masks the original URL or presents a click-through page instead of a normal HTTP redirect.

When a supported wrapper is detected, the result can show:

- original rewritten address
- security-link provider
- revealed destination
- subsequent HTTP/browser redirects
- true final address

Reputation, domain, TLS and screenshot checks are then targeted at the revealed/final website where possible.
...
A minimal working GitHub repository contains:
...
`packages.txt` installs system Chromium on Streamlit Community Cloud. The application also uses the Playwright Python package for browser automation.

Your real API keys must stay in Streamlit Cloud **App settings -> Secrets** and must not be committed to GitHub.
